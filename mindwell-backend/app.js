...
app.use('/api', (req, res, next) => {
  ...
  if (mongoose.connection.readyState !== 1) {
    return res.status(503).json({
      success: false,
      message: 'Database not connected. Check MONGO_URI in mindwell-backend/.env and restart the backend.'
    });
  }
  next();
});
...
// ✅ Session routes - specific routes defined INSIDE session.routes.js
