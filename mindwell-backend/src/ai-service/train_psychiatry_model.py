...
print("=" * 60)
print("  PSYCHIATRY DIAGNOSIS MODEL TRAINER")
...
# LOAD CSV
# ─────────────────────────────────────────
CSV_FILE = "psychiatry_dataset_final.csv"
...
with open("diag_encoder.pkl", "wb") as f: pickle.dump(diag_encoder, f)
...
model.save_pretrained("./psych_diag_model")
tokenizer.save_pretrained("./psych_diag_model")
print("Model saved to ./psych_diag_model")

# ─────────────────────────────────────────
# PREDICT FUNCTION
# CHANGE 1: confidence threshold = 50%
# CHANGE 2: added "status" column to cols
# CHANGE 3: save ALL predictions (no skip)
# ─────────────────────────────────────────
CONFIDENCE_THRESHOLD = 50.0
PREDICTIONS_FILE = "psychiatry_predictions_log.csv"
...
def predict(sentence, session_type="interactive"):
    ...
    no_issue = (names[0] == "Normal") or (confs[0] < CONFIDENCE_THRESHOLD)
    ...
    if session_type == "interactive":
        ...
        if no_issue:
            ...
            print(f"    {names[0]:<22} {confs[0]:>5.1f}%  {make_bar(confs[0])}")
            print(f"    {names[1]:<22} {confs[1]:>5.1f}%  {make_bar(confs[1])}")
            print(f"    {names[2]:<22} {confs[2]:>5.1f}%  {make_bar(confs[2])}")
...
print("  Type your sentence | Type 'quit' to exit")
print("=" * 60)
