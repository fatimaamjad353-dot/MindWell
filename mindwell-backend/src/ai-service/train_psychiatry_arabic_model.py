...
print("=" * 60)
print("  ARABIC PSYCHIATRY DIAGNOSIS MODEL TRAINER (80/10/10 SPLIT)")
...
# LOAD CSV
# ─────────────────────────────────────────
CSV_FILE = "psychiatry_dataset_arabic_excel.csv"
...
with open("diag_encoder_arabic.pkl", "wb") as f: pickle.dump(diag_encoder, f)
...
args = TrainingArguments(
    output_dir="./psych_diag_arabic_model",
    num_train_epochs=6,
    learning_rate=2e-5,
...
model.save_pretrained("./psych_diag_arabic_model")
tokenizer.save_pretrained("./psych_diag_arabic_model")
print("Model saved to ./psych_diag_arabic_model")

# ─────────────────────────────────────────
# PREDICT FUNCTION
# Always shows diagnosis — no confidence threshold filtering
# ─────────────────────────────────────────
PREDICTIONS_FILE = "psychiatry_arabic_predictions_log.csv"
cols = ["patient_statement","predicted_diagnosis","confidence_%","top2","top2_%","top3","top3_%","session_type","timestamp"]
...
def predict(sentence, session_type="interactive"):
    ...
    if session_type == "interactive":
        ...
        print(f"    {names[0]:<30} {confs[0]:>5.1f}%  {make_bar(confs[0])}")
        print(f"    {names[1]:<30} {confs[1]:>5.1f}%  {make_bar(confs[1])}")
        print(f"    {names[2]:<30} {confs[2]:>5.1f}%  {make_bar(confs[2])}")
...
print("  Type your Arabic statement | Type 'quit' to exit")
print("=" * 60)

while True:
    try:
        user_input = input("\nEnter Arabic patient statement: ").strip()
