...
import { getSessionRoom, saveSessionRoom } from '../utils/sessionRoomStorage';

export default function TextSessionScreen({ navigation, route }) {
  const { role = 'patient', session = {} } = route.params || {};
  const sessionId = String(session.id || 'demo-text-session');
  ...
  const addPrescription = async () => {
    if (medicine.trim().length < 2 || instructions.trim().length < 5) {
      Alert.alert('Prescription incomplete', 'Enter the medicine and clear usage instructions.');
      return;
    }
    ...
    setMedicine('');
    setInstructions('');
    Alert.alert('Prescription shared', 'The patient can now view it in this text session.');
  };
    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          <Text style={styles.headerTitle}>Text Session</Text>
          <Text style={styles.headerSub}>{otherPerson} - Private session chat</Text>
        </View>
      ...
      >
        <View style={styles.sessionNotice}>
          <Text style={styles.noticeTitle}>Session-only text room</Text>
          <Text style={styles.noticeText}>
            Use this screen for booked text sessions. Both patient and psychiatrist see the same room.
          </Text>
        </View>

        <Text style={styles.sectionTitle}>Messages</Text>
        <View style={styles.chatCard}>
          {room.messages.length ? room.messages.map(item => {
            ...
            <Text style={styles.emptyText}>No messages yet. Start the text session conversation.</Text>
          )}
        </View>
        ...
        {role === 'psychologist' && (
          <View style={styles.prescriptionForm}>
            ...
            <TextInput style={styles.formInput} value={medicine} onChangeText={setMedicine} placeholder="Medicine and strength" placeholderTextColor="#aaa" />
        ...
        {room.prescriptions.length ? room.prescriptions.map(item => (
          <View key={item.id} style={styles.prescriptionCard}>
            <Text style={styles.medicine}>{item.medicine}</Text>
            <Text style={styles.instructions}>{item.instructions}</Text>
            <Text style={styles.prescriptionDate}>Shared {new Date(item.createdAt).toLocaleDateString()}</Text>
          </View>
        )) : (
          <View style={styles.noPrescription}>
            <Text style={styles.noPrescriptionText}>
              {role === 'psychologist'
                ? 'No prescription has been shared in this text session.'
                : 'Your psychiatrist has not shared a prescription in this text session.'}
            </Text>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  chatCard: { minHeight: 180, backgroundColor: '#fff', borderRadius: 14, padding: 12, elevation: 2 },
  emptyText: { color: '#888', fontSize: 12, textAlign: 'center', marginTop: 58 },
